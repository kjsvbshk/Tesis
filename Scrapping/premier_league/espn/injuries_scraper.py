#!/usr/bin/env python3
"""
Scraper de lesiones de jugadores de la Premier League desde ESPN.
"""
...
def scrape_injuries(date=None):
    """
    Extraer reportes de lesiones de jugadores de la Premier League.
    ...
    """
    # URL de lesiones de Premier League (puede variar según ESPN)
    url = "https://www.espn.com/soccer/team/injuries/_/league/ENG.1"
    
    try:
        if not date:
            date = datetime.now().strftime('%Y-%m-%d')
        
        logger.info(f"Scrapeando reportes de lesiones desde: {url}")
        ...
        injuries_data = extract_injuries_data(soup, date)
...
def extract_injuries_data(soup, date):
    ...
    try:
        ...
        for container in injury_containers:
            ...
            team_injuries = extract_team_injuries(container, team_name, date)
...
def extract_team_name_from_container(container):
    ...
    try:
        ...
        # Buscar patrones comunes de nombres de equipos de Premier League
        premier_league_teams = [
            "Arsenal", "Aston Villa", "Brentford", "Brighton", "Burnley", "Chelsea",
            "Crystal Palace", "Everton", "Leeds", "Leicester", "Liverpool", "Manchester City",
            "Manchester United", "Newcastle", "Norwich", "Southampton", "Tottenham",
            "Watford", "West Ham", "Wolves", "Wolverhampton"
        ]
        
        for team in premier_league_teams:
            if team in container_text:
                return team
...
def extract_team_injuries(container, team_name, date):
    """
    ...
    try:
        ...
        for row in rows:
            ...
            injury_data = parse_injury_row(row, team_name, date)
            if injury_data:
...
def parse_injury_row(row, team_name, date):
    """
    ...
    try:
        ...
        injury_data = {
            "date": date,
            "team_name": team_name,
            "player_name": extract_player_name(cells[0]) if len(cells) > 0 else None,
            "position": extract_position(cells[1]) if len(cells) > 1 else None,
            "status": extract_status(cells[2]) if len(cells) > 2 else None,
            "injury_type": extract_injury_type(cells[3]) if len(cells) > 3 else None,
            "expected_return": extract_expected_return(cells[4]) if len(cells) > 4 else None
        }
        
        # Solo incluir si tiene al menos jugador y estado
        if injury_data["player_name"] and injury_data["status"]:
            return injury_data
        
        return None
        
    except Exception as e:
        logger.warning(f"Error al parsear fila de lesión: {e}")
        return None
...
def extract_injury_type(cell):
    """Extraer tipo de lesión."""
    try:
        text = cell.get_text().strip()
        if text:
            return text
...
def extract_expected_return(cell):
    """Extraer fecha esperada de regreso."""
    try:
        text = cell.get_text().strip()
        if text:
            return text
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Scraper de lesiones de jugadores de Premier League desde ESPN'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Fecha del reporte en formato YYYY-MM-DD. Si no se especifica, usa fecha actual'
    )
    
    args = parser.parse_args()
    
    # Scrapear lesiones
    injuries_data = scrape_injuries(date=args.date)
    
    if injuries_data:
        # Guardar CSV
        save_injuries_to_csv(injuries_data, date=args.date)
        logger.info("Scraping de lesiones completado exitosamente")
    else:
